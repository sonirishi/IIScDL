...
class DataReader:
    ...
    def data_to_tensor(self,loc,filename,bs,seed):
        ...
        train_x_scale = torch.Tensor(train_x_scale.astype(np.float32))
        train_y_oh = torch.Tensor(train_y_oh)
        val_x_scale = torch.Tensor(val_x_scale.astype(np.float32))
        val_y_oh = torch.Tensor(val_y_oh)
        test_x_scale = torch.Tensor(test_x_scale.astype(np.float32))
...
ax.scatter(xs=train_acc_store[:,1], ys=train_acc_store[:,0], \
                zs=train_acc_store[:,21], cmap = cm.coolwarm, s=30, c=train_acc_store[:,21])
...
fig = plt.figure(figsize=(10,8))
ax = fig.gca(projection='3d')
ax.scatter(xs=valid_acc_store[:,1], ys=valid_acc_store[:,0], \
                zs=valid_acc_store[:,21], cmap = cm.coolwarm, s=30, c=valid_acc_store[:,21])
ax.set_xlabel('BatchSize')
ax.set_ylabel('LearningRate')
ax.set_zlabel('Accuracy')
...
ax.scatter(xs=test_acc_store[:,1], ys=test_acc_store[:,0], \
                zs=test_acc_store[:,21], cmap = cm.coolwarm, s=30, c=test_acc_store[:,21])
ax.set_xlabel('BatchSize')
ax.set_ylabel('LearningRate')
ax.set_zlabel('Accuracy')
...
fig = plt.figure(figsize=(10,8))
plt.plot(train_acc_store[5,2:22],label='Train')
plt.plot(valid_acc_store[5,2:22],label='Validation')
plt.plot(test_acc_store[5,2:22],label='Test')
